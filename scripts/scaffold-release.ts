#!/usr/bin/env bun
// scaffold-release.ts — Bump scaffold version and create a git tag
//
...
export function bumpScaffoldVersion(type: BumpType, root: string): string {
  const pkgPath = path.join(root, "package.json");
  const pkg = JSON.parse(readFileSync(pkgPath, "utf-8"));
  const current = pkg.scaffold?.version || "0.0.0";
  const newVersion = bumpVersion(current, type);
  pkg.scaffold = pkg.scaffold || {};
  pkg.scaffold.version = newVersion;
...
if (import.meta.main) {
  ...
  if (!bumpType) {
    console.error("[release] Usage: scaffold-release.ts <major|minor|patch>");
    process.exit(1);
  }
  ...
  const newVersion = bumpScaffoldVersion(bumpType, root);
  const tagName = `scaffold-v${newVersion}`;
  ...
  if (bumpType === "major") {
    ...
    console.log("[release]    Games should run: git fetch scaffold && git merge scaffold/main");
    console.log("");
  }
