#!/usr/bin/env bun
// scaffold-verify.ts — Run verification checks after a scaffold sync
//
// Usage: bun scripts/scaffold-verify.ts
//
...
function run(): void {
  ...
  for (const step of STEPS) {
    ...
    try {
      ...
      console.error("[verify] Fix the issue above, then re-run: bun run scaffold:verify");
      process.exit(1);
    }
  }
