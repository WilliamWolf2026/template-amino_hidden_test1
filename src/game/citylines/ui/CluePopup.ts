 ...
 *
 * Shows dog head in a circle with speech bubble containing clue text.
 * Positioned at top center of screen.
 * Auto-dismisses after specified duration or on tap.
 */
export class CluePopup extends Container {
  ...
  // Configurable sizes
  private readonly circleSize = 64;
  private readonly borderWidth = 4;
  private readonly borderColor = 0x4A3728;
  private readonly dialogWidth = 280;
  private readonly dialogGap = 8; // Gap between circle and dialog
  ...
  constructor(gpuLoader: PixiLoader) {
    ...
    // Create character sprite (news_hound)
    const spriteName = CHARACTER_SPRITES.news_hound;
    const sprite = gpuLoader.createSprite(getAtlasName(), spriteName);
    sprite.anchor.set(0.5);

    // Scale to show head portion in circle
    // Head is roughly top 40% of sprite, scale so head fills the circle
    const headScale = (this.circleSize * 2.2) / CHARACTER_BASE_SIZE.height;
    ...
    const dialogueTexture = gpuLoader.getTexture(getAtlasName(), 'dialogue.png');
  ...
  show(
    ...
    gsap.to(this.scale, {
      ...
      onComplete: () => {
        ...
        this.clickArea.eventMode = 'static'; // Enable clicks after animation
      },
    });
  ...
  dismiss(): void {
    ...
    this.isAnimating = true;
    this.clickArea.eventMode = 'none'; // Disable clicks during animation
