...
See also: [entry-point-map.md](entry-point-map.md), [scene-graph.md](scene-graph.md), [context-map.md](context-map.md)
...
graph TB
    %% Entry & Providers
    subgraph ENTRY["Entry (Vite)"]
        direction TB
        IDX["index.html"] --> APP["app.tsx"]
    end

    subgraph PROVIDERS["Provider Chain"]
        direction TB
        GB["GlobalBoundary"] --> TP["TuningProvider"]
        TP --> ANA["AnalyticsProvider"]
        ANA --> FF["FeatureFlagProvider"]
        FF --> VMW["ViewportModeWrapper"]
        VMW --> PP["PauseProvider"]
    ...
    subgraph GS_STAGE["GameScreen Stage"]
        ...
        subgraph GAME["CityLinesGame"]
            ...
            G_DECO --> G_DECSP["Sprite xN\ntree/bush/flower"]
            G_LAND["landmarksContainer"]
            G_LAND --> G_LM["Landmark xN"]
    ...
    style IDX fill:#2d5f8a,color:#fff
    style APP fill:#2d5f8a,color:#fff
    style GB fill:#345e80,color:#fff
    style TP fill:#345e80,color:#fff
    style ANA fill:#345e80,color:#fff
    style FF fill:#345e80,color:#fff
    style VMW fill:#345e80,color:#fff
