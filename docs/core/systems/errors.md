...
The core system provides three layers of error boundaries, each handling different scopes of failure.
...
```typescript
import { GlobalBoundary } from '~/core/systems/errors';
...
```typescript
import { ScreenBoundary } from '~/core/systems/errors';
...
```typescript
import { AssetBoundary } from '~/core/systems/errors';
...
```typescript
import { reportError } from '~/core/systems/errors';
...
Configure error tracking in `src/core/config.ts`:
