...
SolidJS signals for reactive state across the core system.

## Overview

The core system uses SolidJS signals for all reactive state. Signals provide fine-grained reactivity without the overhead of a virtual DOM.
...
The core system auto-binds spacebar to pause toggle:

```typescript
// In app.tsx
import { initPauseKeyboard, cleanupPauseKeyboard } from '~/core/systems/pause';
...
## Modules Tier Awareness

Modules (primitives, prefabs, logic) maintain their own internal state using the same signal patterns. When a module exposes reactive state, it follows the `createRoot` pattern to persist across component lifecycles. Module state is accessed through each module's public API rather than through a global store.
